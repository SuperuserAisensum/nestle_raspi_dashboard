...
async function viewEventDetails(eventId) {
    try {
        ...
        const data = await response.json();
        console.log('Event details:', data); // Debug log to see all data
        ...
        if (iqiElement) {
            iqiElement.innerHTML = `
                ...
                <div class="font-semibold text-gray-800 text-xl flex items-center">
                    ${data.iqi_score ? parseFloat(data.iqi_score).toFixed(2) : 'NaN'}
        ...
        const imageElement = eventImage.querySelector('img');
        if (data.image_path) {
            // Ensure we're using the correct path
            imageElement.src = '/' + data.image_path;
            imageElement.classList.remove('hidden');
            
            // Create a canvas to draw bounding boxes if we have raw_detections
            if (data.raw_detections && data.raw_detections.length > 0) {
                console.log('Drawing bounding boxes for detections:', data.raw_detections);
                
                // Wait for the image to load before drawing boxes
                imageElement.onload = function() {
                    // Check if we already have a canvas, remove it if it exists
                    const existingCanvas = eventImage.querySelector('canvas');
                    if (existingCanvas) {
                    ...
                    // Set the container to position relative if it's not already
                    if (eventImage.style.position !== 'relative') {
                        eventImage.style.position = 'relative';
                    }
                    
                    // Add canvas to the container after the image
                    eventImage.appendChild(canvas);
                    ...
                    data.raw_detections.forEach(detection => {
                        // Determine if it's a Nestle product (green) or competitor (red)
                        const color = detection.is_nestle ? 'green' : 'red';
                        
                        // Use the specified color if available
                        const boxColor = detection.bounding_box_color || color;
                        ...
                        if (detection.x !== undefined && detection.y !== undefined && 
                            ...
                            // Format: {x, y, width, height} where x,y is the center
                            x = detection.x;
                            y = detection.y;
                            ...
                            x = x - width/2;
                            y = y - height/2;
                        } else if (detection.bbox !== undefined) {
                            // Format: {bbox: [x, y, width, height]}
                            [x, y, width, height] = detection.bbox;
                        } else if (Array.isArray(detection.box)) {
                            // Format: {box: [x1, y1, x2, y2]} (top-left and bottom-right)
                            const [x1, y1, x2, y2] = detection.box;
                            x = x1;
                            y = y1;
                            width = x2 - x1;
                            height = y2 - y1;
                        } else if (detection.box?.x1 !== undefined) {
                            ...
                            console.warn('Unknown bounding box format for detection:', detection);
                        ...
                        // Draw class name
                        const displayName = detection.display_name || detection.class || detection.category || 'Unknown';
                        const confidence = detection.confidence ? Math.round(detection.confidence * 100) + '%' : '';
                        const text = `${displayName} ${confidence}`.trim();
                        
                        ctx.fillStyle = boxColor;
                        ctx.fillRect(pixelX, pixelY - 25, ctx.measureText(text).width + 10, 25);
                        ctx.fillStyle = 'white';
                        ctx.font = '16px Arial';
                        ctx.fillText(text, pixelX + 5, pixelY - 5);
                    });
                };
            }
        } else {
