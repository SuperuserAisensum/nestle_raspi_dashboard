...
try:
    ...
    logger.info("Roboflow models initialized successfully")
except Exception as e:
    logger.error(f"Error initializing Roboflow: {str(e)}")
    logger.warning("Running with limited functionality - object detection will be disabled")
...
def filter_overlapping_boxes(boxes, threshold=0.7):
    """Filter out overlapping bounding boxes, keeping the one with highest confidence."""
    if not boxes:
        return []
    ...
    for box in sorted_boxes:
        # For boxes in the format {'box': [x1, y1, x2, y2], ...}
        if 'box' in box:
            current_box = box['box']
            should_keep = True
            
            # Check against all boxes we've already decided to keep
            for kept_box in filtered_boxes:
                if 'box' in kept_box:
                    # Calculate IoU (Intersection over Union)
                    x1_min, y1_min, x1_max, y1_max = current_box
                    
                    kept_coords = kept_box['box']
                    x2_min, y2_min, x2_max, y2_max = kept_coords
    ...
    try:
        ...
        if '_IQI' in filename:
            try:
                ...
                logger.error(f"Error parsing IQI from filename: {str(e)}")
                pass

        # Also try to get IQI score from request.form
        if iqi_score is None and 'iqi_score' in request.form:
            try:
                iqi_score = float(request.form.get('iqi_score'))
            except Exception as e:
                logger.error(f"Error parsing IQI from request.form: {str(e)}")
                pass

        # Additional check: try to get IQI score from roboflow_outputs
        if iqi_score is None and 'roboflow_outputs' in request.form:
            try:
                roboflow_data = json.loads(request.form.get('roboflow_outputs'))
                if 'iqi_score' in roboflow_data:
                    iqi_score = float(roboflow_data['iqi_score'])
            except Exception as e:
                logger.error(f"Error extracting IQI score from roboflow_outputs: {str(e)}")
        ...
        # Perform object detection on the server, similar to check_image
        # Calculate IQI score if not provided
        ...
        if yolo_model is not None:
            try:
                ...
                logger.error(f"Error with Roboflow detection: {str(e)}")
                logger.warning("Continuing without Nestle product detection")
        else:
            logger.warning("Skipping Roboflow detection - model not available")
        
        # OWLv2 detection for competitor products
        competitor_products = {}
        competitor_boxes = []
        total_competitor = 0
            ...
            if 'data' in owlv2_result and owlv2_result['data']:
                for obj in owlv2_result['data'][0]:
                    if 'bounding_box' in obj:
                        ...
                        # Only check overlap if we have nestle boxes
                        should_include = True
                        if nestle_boxes:
                            should_include = not is_overlap(bbox, [(box['x'], box['y'], box['width'], box['height']) for box in nestle_boxes])
                ...
                # Filter out overlapping competitor boxes to avoid duplicates
                competitor_boxes = filter_overlapping_boxes(competitor_boxes)
                
                # Count the filtered boxes
                total_competitor = len(competitor_boxes)
                competitor_products = {}
                for box in competitor_boxes:
                    category = box.get('class', 'unclassified')
                    competitor_products[category] = competitor_products.get(category, 0) + 1
        except Exception as e:
            logger.error(f"Error with OWLv2 API: {str(e)}")
            logger.warning("Skipping competitor detection")
        ...
        # Combine all detections for raw_detections
        raw_detections = []
        for box in nestle_boxes:
        ...
        with get_db_connection() as conn:
            ...
            detection_data = {
                ...
                'raw_detections': raw_detections  # Add raw_detections to the saved data
            }
            ...
            cursor = conn.execute(
                ...
                (device_id, timestamp, json.dumps(detection_data), 
                 labeled_path, current_time, iqi_score, address)
            )
...
def get_event_details(event_id):
    try:
        with get_db_connection() as conn:
            ...
            # IMPORTANT: Add raw_detections to the response
            raw_detections = []
            if isinstance(roboflow_data, dict):
                # Check for raw_detections in the roboflow_outputs
                if 'raw_detections' in roboflow_data:
                    raw_detections = roboflow_data['raw_detections']
                # Also check for predictions which might have the bounding box data
                elif 'predictions' in roboflow_data:
                    raw_detections = roboflow_data['predictions']
            ...
            response = {
                ...
                'raw_detections': raw_detections  # Add raw_detections to the response
            }
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                # Check if Roboflow model is available
                if yolo_model is not None:
                    # Use Roboflow for object detection
